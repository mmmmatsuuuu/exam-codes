 ...
 ***************************************************************************/
const Namae    = ["捨て要素", "A", "B", "C", "D", "E", "F", "G", "H", "I", "J"];
const Kibo     = [
    ["捨て配列"],
    ["捨て要素", 1, 3, 2, 4],
    ["捨て要素", 3, 1, 4, 2],
...
const Kibosu   = ["捨て要素", 0, 0, 0, 0];
const Gteiin   = ["捨て要素", 0, 0, 0, 0];

/***************************************************************************
* 方法２の実装
***************************************************************************/
const syo   = Math.floor(ninzu / 4);
const amari = ninzu % 4;
...
for (let i=1; i<=amari; i++) {
    var s = -1;
    for (let j=1; j<=4; j++) {
        if (s < Kibosu[j] && Gteiin[j] == syo) {
            s = Kibosu[j];
            g = j;
        }
    }
...
// ここから下は問題には記述されていない。
for (let i=1; i<=4; i++) {
    console.log("グループ" + i + "のメンバー: ");
    for (let j=1; j<=Gninzu[i]; j++) {
        console.log(Huriwake[i][j]);
    }
}